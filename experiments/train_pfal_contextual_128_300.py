...
def load_rl_params(config_path: str = None) -> dict:
    ...
    if config_path is None:
        config_path = PROJECT_ROOT / "configs" / "rl_params_128_300.yaml"
...
class CollectorInfoSanitizer(gym.Wrapper):
    """Keep Collector-facing info payloads shape-stable across vector envs."""
...
def _make_env(
    ...
    def _fn():
        ...
        return CollectorInfoSanitizer(env)
    return _fn
...
def evaluate_on_schedules(
    ...
    if schedules is None:
        schedules = select_evaluation_schedules(
            ...
            bounds=load_schedule_bounds(
                str(PROJECT_ROOT / "configs" / "schedule_params.yaml")
            ),
        )
    else:
    ...
    controller_mode = str(controller_mode or "rl").lower()
    if controller_mode == "rl":
        if policy is None:
            raise ValueError("evaluate_on_schedules requires a policy when controller_mode='rl'.")
        policy.eval()
    ...
    for i, sched in enumerate(schedules):
        ...
        for rep in range(n_episodes_per_schedule):
            ...
            if controller_mode == "pid":
                env.action_semantics = "absolute"
                env._action_anchor_controller = None
            reset_options = {
                "schedule": sched,
            ...
            if controller_mode == "pid":
                pid_controller = PFALConventionalController(env)
            ...
            while not done:
                if controller_mode == "rl":
                    ...
                    action = np.asarray(
                        pid_controller.predict(obs, context=None),
                        dtype=np.float32,
                    ).ravel()[:5]
        ...
        per_schedule_outcomes.append(
            {
                ...
                "mean_reward_per_day": float(np.mean(sched_reward_per_day)),
                "mean_harvest_g": float(mean_h),
                "mean_harvest_g_per_day": float(np.mean(sched_harvest_per_day)),
                "mean_cost": float(mean_c),
                "mean_cost_per_day": float(np.mean(sched_cost_per_day)),
...
def compare_eval_results(
    ...
    candidate_label: str = "rl",
    baseline_label: str = "pid",
) -> dict:
    """Create a compact, schedule-aligned comparison summary."""
    ...
    overall_delta = {
        name: float(candidate_results.get(name, 0.0) - baseline_results.get(name, 0.0))
        for name in summary_metrics
    }

    candidate_by_key = {
        _schedule_key(item["schedule"]): dict(item)
        for item in candidate_results.get("per_schedule_outcomes", [])
    }
    baseline_by_key = {
        _schedule_key(item["schedule"]): dict(item)
        for item in baseline_results.get("per_schedule_outcomes", [])
    }
    shared_keys = [key for key in candidate_by_key.keys() if key in baseline_by_key]

    per_schedule = []
    win_counts = {
        "reward_per_day_higher": 0,
        "harvest_higher": 0,
        "cost_lower": 0,
        "constraint_cost_lower": 0,
        "completion_ratio_higher": 0,
        "early_termination_lower": 0,
        "safety_override_lower": 0,
    }
    for key in shared_keys:
        ...
        entry = {
            "schedule_key": key,
            "schedule": dict(cand["schedule"]),
            "candidate_reward_per_day": float(cand.get("mean_reward_per_day", 0.0)),
            "baseline_reward_per_day": float(base.get("mean_reward_per_day", 0.0)),
            "delta_reward_per_day": float(
                cand.get("mean_reward_per_day", 0.0) - base.get("mean_reward_per_day", 0.0)
            ),
            "candidate_harvest_g": float(cand.get("mean_harvest_g", 0.0)),
            "baseline_harvest_g": float(base.get("mean_harvest_g", 0.0)),
            "delta_harvest_g": float(
                cand.get("mean_harvest_g", 0.0) - base.get("mean_harvest_g", 0.0)
            ),
            "candidate_cost_per_day": float(cand.get("mean_cost_per_day", 0.0)),
            "baseline_cost_per_day": float(base.get("mean_cost_per_day", 0.0)),
            "delta_cost_per_day": float(
                cand.get("mean_cost_per_day", 0.0) - base.get("mean_cost_per_day", 0.0)
            ),
            "candidate_constraint_cost": float(cand.get("mean_constraint_cost", 0.0)),
            "baseline_constraint_cost": float(base.get("mean_constraint_cost", 0.0)),
            "delta_constraint_cost": float(
                cand.get("mean_constraint_cost", 0.0) - base.get("mean_constraint_cost", 0.0)
            ),
            "candidate_completion_ratio": float(cand.get("mean_completion_ratio", 0.0)),
            "baseline_completion_ratio": float(base.get("mean_completion_ratio", 0.0)),
            "delta_completion_ratio": float(
                cand.get("mean_completion_ratio", 0.0) - base.get("mean_completion_ratio", 0.0)
            ),
            "candidate_early_termination_ratio": float(
                cand.get("early_termination_ratio", 0.0)
            ),
            "baseline_early_termination_ratio": float(
                base.get("early_termination_ratio", 0.0)
            ),
            "delta_early_termination_ratio": float(
                cand.get("early_termination_ratio", 0.0)
                - base.get("early_termination_ratio", 0.0)
            ),
            "candidate_safety_override_count": float(
                cand.get("mean_safety_override_count", 0.0)
            ),
            "baseline_safety_override_count": float(
                base.get("mean_safety_override_count", 0.0)
            ),
            "delta_safety_override_count": float(
                cand.get("mean_safety_override_count", 0.0)
                - base.get("mean_safety_override_count", 0.0)
            ),
        }
        per_schedule.append(entry)
        win_counts["reward_per_day_higher"] += int(entry["delta_reward_per_day"] > 0.0)
        win_counts["harvest_higher"] += int(entry["delta_harvest_g"] > 0.0)
        win_counts["cost_lower"] += int(entry["delta_cost_per_day"] < 0.0)
        win_counts["constraint_cost_lower"] += int(entry["delta_constraint_cost"] < 0.0)
        win_counts["completion_ratio_higher"] += int(entry["delta_completion_ratio"] > 0.0)
        win_counts["early_termination_lower"] += int(
            entry["delta_early_termination_ratio"] < 0.0
        )
        win_counts["safety_override_lower"] += int(
            entry["delta_safety_override_count"] < 0.0
        )
    ...
    return {
        ...
        "shared_schedule_count": int(len(shared_keys)),
        "win_counts": win_counts,
        "candidate_controller_mode": str(candidate_results.get("controller_mode", "")),
        "baseline_controller_mode": str(baseline_results.get("controller_mode", "")),
        "candidate_termination_reason_shares": dict(
            candidate_results.get("termination_reason_shares", {})
        ),
        "baseline_termination_reason_shares": dict(
            baseline_results.get("termination_reason_shares", {})
        ),
...
def train(
    ...
    result = offpolicy_trainer(
        ...
        step_per_collect=resolved_step_per_collect,
        ...
        logger=logger,
        update_per_step=resolved_update_per_step,
        test_in_train=False,
        verbose=False,
    )
...
def main():
    ...
    if args.eval_only:
        ...
        eval_schedules = select_evaluation_schedules(
            n_schedules=args.n_eval_schedules,
            seed=args.eval_seed,
            context_sampling_phase=params.get("context_sampling_phase", "full"),
            fixed_schedule=params.get("context_fixed_schedule"),
            narrow_bounds=params.get("context_narrow_bounds"),
            selection_strategy=params.get("eval_schedule_selection", "coverage"),
        ...
        pid_eval_results = evaluate_on_schedules(
            None,
            n_schedules=len(eval_schedules),
            n_episodes_per_schedule=args.n_eval_episodes_per_schedule,
            seed=args.eval_seed,
            context_sampling_phase=params.get("context_sampling_phase", "full"),
            fixed_schedule=params.get("context_fixed_schedule"),
        ...
        eval_vs_pid = compare_eval_results(
            eval_results,
            pid_eval_results,
            candidate_label="rl",
            baseline_label="pid",
        ...
        print(f"PID reward/day  : {pid_eval_results['mean_reward_per_day']:.4f}")
        print(f"PID cost/day    : {pid_eval_results['mean_cost_per_day']:.4f}")
        print(f"PID constraint  : {pid_eval_results['mean_constraint_cost']:.4f}")
        print(f"RL-PID reward/day delta : {eval_vs_pid['overall_delta']['mean_reward_per_day']:.4f}")
        print(f"RL-PID cost/day delta   : {eval_vs_pid['overall_delta']['mean_cost_per_day']:.4f}")
        print(f"RL-PID constraint delta : {eval_vs_pid['overall_delta']['mean_constraint_cost']:.4f}")
        ...
        attach_inlet_seedling_metadata(pid_eval_results, params)
        save_json_report(
            str(eval_output_dir / "baseline_pid_generalization_eval_eval_only.json"),
            pid_eval_results,
        )
        attach_inlet_seedling_metadata(eval_vs_pid, params)
        save_json_report(
            str(eval_output_dir / "generalization_vs_pid_eval_only.json"),
            eval_vs_pid,
        )
        print("=" * 60)
        return
    ...
    attach_inlet_seedling_metadata(pid_eval_results, params)
    save_json_report(
        os.path.join(log_path, "baseline_pid_generalization_eval.json"),
        pid_eval_results,
    )
    eval_vs_pid = compare_eval_results(
        eval_results,
    ...
    attach_inlet_seedling_metadata(eval_vs_pid, params)
    save_json_report(
        os.path.join(log_path, "generalization_vs_pid.json"),
        eval_vs_pid,
    )
    selected_eval_results = None
    selected_vs_pid = None
    selected_policy_path = os.path.join(log_path, "policy_selected.pth")
    if os.path.exists(selected_policy_path):
        ...
        selected_vs_pid = compare_eval_results(
            selected_eval_results,
            pid_eval_results,
            candidate_label="selected_rl",
            baseline_label="pid",
        )
        attach_inlet_seedling_metadata(selected_vs_pid, params)
        save_json_report(
            os.path.join(log_path, "generalization_selected_vs_pid.json"),
            selected_vs_pid,
        )
        print(
    ...
    print(f"RL-PID cost/day delta   : {eval_vs_pid['overall_delta']['mean_cost_per_day']:.4f}")
    print(f"RL-PID constraint delta : {eval_vs_pid['overall_delta']['mean_constraint_cost']:.4f}")
    if selected_vs_pid is not None:
        print(
            f"Selected-PID reward/day delta : "
            f"{selected_vs_pid['overall_delta']['mean_reward_per_day']:.4f}"
        )
        print(
            f"Selected-PID cost/day delta   : "
            f"{selected_vs_pid['overall_delta']['mean_cost_per_day']:.4f}"
        )
